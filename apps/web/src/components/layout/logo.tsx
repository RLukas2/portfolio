...
const Logo = ({ className }: { className?: string }) => {
  ...
  return (
    <Link
      aria-label="Home"
      className={cn('group relative flex items-center gap-2.5 outline-none', className)}
      onClick={handleClick}
      to="/"
    >
      <span className="relative flex size-9 items-center justify-center">
        <span
          aria-hidden="true"
          className="absolute inset-0 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 opacity-0 blur-lg transition-opacity duration-500 group-hover:opacity-60 group-focus-visible:opacity-60"
        />
        ...
        <span className="relative flex size-9 items-center justify-center rounded-xl border border-white/10 bg-gradient-to-br from-zinc-900 to-zinc-800 shadow-black/20 shadow-lg ring-1 ring-white/5 ring-inset transition-transform duration-300 group-hover:scale-105 group-focus-visible:scale-105 dark:from-zinc-800 dark:to-zinc-900">
          <span aria-hidden="true" className="relative flex items-center font-mono text-sm tracking-tighter">
